...
flags.DEFINE_integer('prediction_time', 2764800, 'Prediction time.')
    ...
    with open(filename) as file:
        for line in file:
            ...
            if 'weibo' in FLAGS.input:
                ...
                # month = int(time.strftime('%m', time.gmtime(float(parts[2]))))
                # day = int(time.strftime('%d', time.gmtime(float(parts[2]))))
                east8_timestamp = float(parts[2]) + 28800
                month = int(time.strftime('%m', time.gmtime(east8_timestamp)))
                day = int(time.strftime('%d', time.gmtime(east8_timestamp)))
